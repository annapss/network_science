...
try:
    ...
    # ... (após a linha: df_filtrado['Autor_Canonico'] = df_filtrado['Autores'].map(mapeamento_nomes))
    
    # =================================================================================
    # NOVO: SALVAMENTO DO ARQUIVO AUXILIAR DE AUTOR X EVENTO PARA O HEATMAP
    # =================================================================================
    print("Salvando arquivo auxiliar Autor x Evento para o Heatmap...")
    df_afiliacao_eventos = df_filtrado[['Autor_Canonico', 'Sigla']].copy()
    
    # Garante que o diretório de dados exista
    import os
    os.makedirs('trabalho_final/dados', exist_ok=True)
    
    artigos_filtrados_file = 'trabalho_final/edge_list/artigos_eventos_por_autor_' + str(ano_inicio) + '-' + str(ano_fim) + '.csv'
    
    # Salvamos a lista Autor_Canonico, Sigla (que é o que o heatmap precisa)
    df_afiliacao_eventos.to_csv(artigos_filtrados_file, index=False)
    print(f"Arquivo auxiliar Autor x Evento salvo em: {artigos_filtrados_file}")
    # =================================================================================
